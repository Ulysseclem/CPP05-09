...
/*   Updated: 2024/02/26 14:07:51 by uclement         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::signForm(Form& form)
{
	if (form.getIsSigned())
	{
		std::cout << CYAN << form.getName() << NC << " is already signed !" << std::endl;
		return ;
	}
	try
	{
		form.beSigned(*this);
		std::cout << getName() << " Signed " << CYAN << form.getName() << NC << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cout << getName() << " couldn't  sign " << CYAN << form.getName() << NC << " because " << e.what() << std::endl;
	}
}
